...
def analyze_video_for_deepfakes(video_path: str, target_fps: int = 2):
  ...
  batch_frames = []
  frame_timestamps = []  # NEW: Array to hold the timestamp for each extracted frame
  frame_count = 0
  ...
  while cap.isOpened():
    ...
    if frame_count % frame_interval == 0:
      # NEW: Calculate the timestamp in seconds for the current frame
      ...
      frame_timestamps.append(timestamp_sec)  # Store the timestamp
  ...
  # NEW: Map predictions back to their timestamps to find the exact fake frames
  suspicious_frame_details = []

  for i, score in enumerate(predictions):
    # Determine if this specific frame is considered fake
    is_fake = False
    if settings.FAKE_IS_LOW_SCORE:
      is_fake = score < settings.THRESHOLD
    else:
    ...
    # If it's a fake frame, record its timestamp and exact score
    if is_fake:
  ...
  return {
    ...
    "suspicious_frame_details": suspicious_frame_details  # Include details in the API response
  }